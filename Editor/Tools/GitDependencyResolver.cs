...
public class GitDependencyResolver
{
    ...
    static void OnPackagesRegistered(PackageRegistrationEventArgs packageRegistrationInfo)
    {
        ...
        foreach (var package in packageRegistrationInfo.changedTo)
        {
            // get the dependencies of the added package
            if (!GetDependencies(package, out var package_dependencies))
            {
                continue;
            }
            ...
            {
                Debug.Log($"Found git depencency {d} in {package.name}");
            }
        }
