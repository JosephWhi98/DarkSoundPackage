...
public class GLDebug : MonoBehaviour
{
    ...
    private struct Text
    {
        ...
        public Text(string text, Vector3 position, float duration)
        {
            this.text = text;
		...
		public bool DurationElapsed(bool drawText)
		{
			if (drawText) 
			{
	...
	public void OnDrawGizmos() 
	{
        if (text != null)
        {
            text = text.Where(l => !l.DurationElapsed(true)).ToList();
        }
	}

    public static void DrawText(string text, Vector3 position, float duration = 0f)
    {
        instance.text.Add(new Text(text, position, duration));
    } 
